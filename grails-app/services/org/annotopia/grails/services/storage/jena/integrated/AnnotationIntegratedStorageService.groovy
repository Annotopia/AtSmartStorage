...
class AnnotationIntegratedStorageService {
	...
	/**
	 * Retrieve the annotation URIs and annotation graph URIs in an annotation set identified by the given URI
	 * @param apiKey	The API key of the client issuing the request
	 * @param uri		The URI of the annotation set
	 * @return  A map of annotation URIs : annotation graph URIs.
	 */
	def retrieveAnnotationUrisInSet(apiKey, uri) {
		log.info '[' + apiKey + '] Retrieving annotations in set ' + uri

		String queryString =
		        "PREFIX at:  <http://purl.org/annotopia#> " +
				"SELECT DISTINCT * WHERE { graph ?g { "+
				  "<"+uri+"> at:annotations ?annotation_graph . " +
				  "graph ?annotation_graph { " +
					"?annotation_uri a <http://www.w3.org/ns/oa#Annotation> " +
				  "}"+
				"}}"
		...
		try {
			...
			while (results.hasNext()) {
				...
				uris.put(result.get("annotation_uri").toString(),
						 result.get("annotation_graph").toString())
			}
			return uris
		} catch (Exception e) {
...
//			updateDateProperty(annotationModel, annotationSetUri, PAV.PAV_LAST_UPDATED_ON);
			...
			storedDataset.addNamedModel(graphUri, annotationModel);
	...
	private void persistNewAnnotation(def apiKey, def annotationUris, Model annotationModel, HashMap<Resource, String> oldNewAnnotationUriMapping) {
		...
		while(annotationUrisIterator.hasNext()) {
			...
			log.info("[" + apiKey + "] Minting URI for annotation " + annotation.toString());
